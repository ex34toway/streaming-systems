...
从概念上讲，处理边界数据非常简单，很可能大家都比较熟悉。在图1-2中，我们从左侧开始，有一个完整的数据集的熵。我们通过一些数据处理引擎（通常是批处理，尽管精心设计的流式处理引擎也可以正常工作），例如MapReduce，然后获得右侧一个新的结构化数据集，通该数据集具有更大的内在价值。
